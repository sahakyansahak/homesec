...
def s_sys():
	...
	if 1:
		return jsonify(a, 
			sqlite3.connect("DB/evth.db").cursor().execute("SELECT * FROM solar1").fetchall()[0], 
...
def new_day(my_day):
	...
	for i in c.execute("SELECT * FROM t1").fetchall():
		...
		if int(day[1]) == int(get_day[1]) and int(day[2]) == int(get_day[0]) and int(day[3]) == int(get_day[2]):
			temps.append(i[1])
